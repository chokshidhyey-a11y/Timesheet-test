...
const SEED_ENTRIES = [
  { id: "t1", employeeId: "E001", employeeName: "Marcus Webb",    date: "2026-04-28", day: "Monday",    job: "J-4421", hours: 6, rnd: false, status: "approved", supervisorId: "S001", notes: "Mould base machining" },
  { id: "t2", employeeId: "E001", employeeName: "Marcus Webb",    date: "2026-04-28", day: "Monday",    job: "J-4421", hours: 2, rnd: true,  status: "approved", supervisorId: "S001", notes: "Testing new toolpath" },
  { id: "t3", employeeId: "E002", employeeName: "Dana Kowalski",  date: "2026-04-29", day: "Tuesday",   job: "J-3310", hours: 8, rnd: false, status: "approved", supervisorId: "S001", notes: "" },
  { id: "t4", employeeId: "E001", employeeName: "Marcus Webb",    date: "2026-04-29", day: "Tuesday",   job: "J-5502", hours: 4, rnd: false, status: "approved", supervisorId: "S001", notes: "Cavity insert" },
  { id: "t5", employeeId: "E001", employeeName: "Marcus Webb",    date: "2026-04-29", day: "Tuesday",   job: "J-5502", hours: 4, rnd: true,  status: "approved", supervisorId: "S001", notes: "R&D on cooling channel" },
  { id: "t6", employeeId: "E003", employeeName: "Troy Hendricks", date: "2026-04-30", day: "Wednesday", job: "J-5502", hours: 6, rnd: false, status: "pending",  supervisorId: "S002", notes: "" },
  { id: "t7", employeeId: "E003", employeeName: "Troy Hendricks", date: "2026-04-30", day: "Wednesday", job: "J-6601", hours: 2, rnd: true,  status: "pending",  supervisorId: "S002", notes: "Prototype fixture test" },
  { id: "t8", employeeId: "E002", employeeName: "Dana Kowalski",  date: "2026-05-01", day: "Thursday",  job: "J-3310", hours: 7, rnd: false, status: "pending",  supervisorId: "S001", notes: "" },
];
...
// ─── TOOLMAKER FORM ──────────────────────────────────────────────────────────
function ToolmakerForm({ user, entries, onSubmit }) {
  ...
  const submit = () => {
    ...
    valid.forEach(r => onSubmit({
      ...
      date, day: dayOfDate(date),
      job: r.job.trim(),
      hours: parseFloat(r.hours), rnd: r.rnd,
      status: "pending", supervisorId: user.supervisor,
      notes: r.comment || ""
    }));
  ...
  return (
    <div className="page">
      ...
      <div className="page-sub">Enter your daily hours — your supervisor will review and approve</div>
      ...
      <div className="card">
        ...
        <div className="form-row">
          ...
          <div className="form-group">
            ...
            <div style={{ padding: "10px 12px", background: "#f8f9fc", border: "1px solid #e4e7f0", borderRadius: 4,
              color: "#c8a84b", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, letterSpacing: 1 }}>
              {date ? dayOfDate(date) : "—"}
            </div>
          </div>
        </div>
          ...
          {jobs.map((row) => (
            <div key={row.id} style={{
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 100px auto 28px", gap: 10, alignItems: "center" }}>
                <input className="form-input" placeholder="Job # e.g. J-4421"
                  value={row.job} onChange={e => updateRow(row.id, "job", e.target.value)} />
                <input className="form-input" type="number" min="0.25" max="24" step="0.25"
                  placeholder="Hrs" value={row.hours}
                  onChange={e => updateRow(row.id, "hours", e.target.value)} />
                <label style={{ display: "flex", alignItems: "center", gap: 7, cursor: "pointer", userSelect: "none" }}>
                  <input type="checkbox" checked={row.rnd}
                    onChange={e => updateRow(row.id, "rnd", e.target.checked)}
                ...
                {jobs.length > 1
                  ? <button className="btn-icon" onClick={() => removeRow(row.id)}>✕</button>
                  : <span />}
              </div>
        ...
        <div style={{ marginTop: 16, background: "#f8f9fc", border: "1px solid #e4e7f0", borderRadius: 6, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr" }}>
            <div style={{ padding: "10px 14px", borderRight: "1px solid #e4e7f0" }}>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: "#9aa0b4", marginBottom: 4 }}>Regular Hrs</div>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 22, fontWeight: 800, color: "#c8a84b" }}>{regHrs.toFixed(2)}</div>
            </div>
            <div style={{ padding: "10px 14px", borderRight: "1px solid #e4e7f0" }}>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: "#9aa0b4", marginBottom: 4 }}>R&D Hrs</div>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 22, fontWeight: 800, color: "#2a8a2a" }}>{rndHrs.toFixed(2)}</div>
            </div>
            <div style={{ padding: "10px 14px" }}>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: "#9aa0b4", marginBottom: 4 }}>Total Hrs</div>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 22, fontWeight: 800, color: "#1a1f2e" }}>{totalHrs.toFixed(2)}</div>
            </div>
          </div>
          {totalHrs > 0 && (
      ...
      <div className="card">
        ...
        {myEntries.length === 0 ? (
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>Date</th><th>Day</th><th>Job</th><th>Hrs</th><th>Type</th><th>Comment</th><th>Status</th></tr></thead>
              <tbody>
                {myEntries.map(e => (
                  <tr key={e.id}>
                    <td>{e.date}</td>
...
function SupervisorView({ user, entries, employees, onUpdate }) {
  ...
  const visible = entries
    .filter(e => myTeam.includes(e.employeeId))
    ...
    <div className="page">
      ...
      <div className="stats-row" style={{ gridTemplateColumns: "repeat(3,1fr)" }}>
        <div className="stat-card">
          ...
          <div className="stat-val" style={{ color: "#c8a84b" }}>{pending}</div>
          <div className="stat-sub">awaiting review</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Team Size</div>
          <div className="stat-val">{myTeam.length}</div>
          <div className="stat-sub">toolmakers</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Total Entries</div>
          <div className="stat-val">{entries.filter(e => myTeam.includes(e.employeeId)).length}</div>
      ...
      <div className="card">
        <div className="table-wrap">
          {visible.length === 0
            ...
            : <table>
                <thead><tr>
                  <th>Employee</th><th>Date</th><th>Day</th><th>Job</th><th>Hrs</th><th>Type</th><th>Comment</th><th>Status</th><th>Action</th>
                </tr></thead>
                <tbody>
                  {visible.map(e => (
                    <tr key={e.id}>
...
function FinanceDashboard({ entries }) {
  ...
  const regHrs    = totalHrs - rndHrs;
  const jobGroups = filtered.reduce((acc, e) => { acc[e.job] = (acc[e.job] || 0) + e.hours; return acc; }, {});

  const exportCSV = () => {
    const rows = [["Employee","Employee ID","Date","Day","Job","Hours","R&D","Comment"]];
    filtered.forEach(e => rows.push([e.employeeName, e.employeeId, e.date, e.day, e.job, e.hours, e.rnd ? "Yes" : "No", e.notes || ""]));
    const csv = rows.map(r => r.map(c => `"${c}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `eurospec-timesheets-${today()}.csv`;
    a.click();
  };
  ...
  return (
    <div className="page">
      ...
      <div className="page-sub">Approved entries only — filter, review, and export</div>

      <div className="stats-row">
        <div className="stat-card">
          <div className="stat-label">Total Hours</div>
          <div className="stat-val">{totalHrs.toFixed(1)}</div>
          <div className="stat-sub">approved + filtered</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Regular Hrs</div>
          <div className="stat-val">{regHrs.toFixed(1)}</div>
          <div className="stat-sub">billable job hours</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">R&D Hrs</div>
          <div className="stat-val" style={{ color: "#2a8a2a" }}>{rndHrs.toFixed(1)}</div>
          <div className="stat-sub">research & development</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Unique Jobs</div>
          <div className="stat-val">{Object.keys(jobGroups).length}</div>
          <div className="stat-sub">job numbers</div>
        </div>
      </div>

      <div className="card">
        <div className="card-title">Filters & Export</div>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "flex-end" }}>
          <div className="form-group" style={{ flex: 1 }}>
            ...
            <input className="form-input" placeholder="e.g. J-4421" value={jobFilter} onChange={e => setJobFilter(e.target.value)} />
          </div>
      ...
      <div className="card">
        <div className="card-title">Hours by Job</div>
        <div className="table-wrap">
          <table>
            <thead><tr><th>Job #</th><th>Type</th><th>Total Hours</th><th>Share</th></tr></thead>
            <tbody>
              {Object.entries(jobGroups).sort((a, b) => b[1] - a[1]).map(([job, hrs]) => {
                const isRnd = filtered.filter(e => e.job === job).every(e => e.rnd);
                return (
                  <tr key={job}>
                    <td style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, letterSpacing: 1, color: "#1a1f2e" }}>{job}</td>
                    <td>{isRnd ? <span className="rnd-badge">R&D</span> : <span style={{ color: "#9aa0b4", fontSize: 11 }}>Regular</span>}</td>
                    <td>{hrs.toFixed(2)}</td>
                    <td>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ flex: 1, background: "#e4e7f0", borderRadius: 2, height: 6, overflow: "hidden" }}>
                          <div style={{ width: `${(hrs / totalHrs * 100).toFixed(0)}%`, background: isRnd ? "#2a8a2a" : "#c8a84b", height: "100%", borderRadius: 2 }} />
                        </div>
                        <span style={{ width: 36, textAlign: "right", color: "#9aa0b4", fontSize: 12 }}>{(hrs / totalHrs * 100).toFixed(0)}%</span>
                      </div>
                    </td>
                  </tr>
                );
      ...
      <div className="card">
        <div className="card-title">All Approved Entries ({filtered.length})</div>
        <div className="table-wrap">
          <table>
            <thead><tr>
              <th>Employee</th><th>ID</th><th>Date</th><th>Day</th><th>Job</th><th>Hours</th><th>Type</th><th>Comment</th>
            </tr></thead>
            <tbody>
              {filtered.slice().reverse().map(e => (
                <tr key={e.id}>
                  <td style={{ color: "#1a1f2e" }}>{e.employeeName}</td>
                  <td style={{ color: "#9aa0b4", fontFamily: "'Barlow Condensed', sans-serif" }}>{e.employeeId}</td>
                  <td>{e.date}</td>
                  <td>{e.day}</td>
                  <td style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, letterSpacing: 1, color: "#1a1f2e" }}>{e.job}</td>
                  <td>{e.hours}</td>
                  <td>{e.rnd ? <span className="rnd-badge">R&D</span> : <span style={{ color: "#9aa0b4", fontSize: 11 }}>Regular</span>}</td>
                  <td style={{ color: "#9aa0b4" }}>{e.notes || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
// ─── ADMIN ────────────────────────────────────────────────────────────────────
function AdminView({ employees, setEmployees, entries }) {
    ...
    <div className="page">
      ...
      <div className="card">
        ...
        <div className="form-row-3">
          ...
          <div className="form-group">
            <label className="form-label">Password</label>
            <input className="form-input" type="text" placeholder="Set password" value={form.password}
              onChange={e => setForm(f => ({ ...f, password: e.target.value }))} />
          </div>
          {form.role === "toolmaker" && (
            <div className="form-group">
              <label className="form-label">Supervisor</label>
              <select className="form-select" value={form.supervisor}
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
          ...
          {editing && (
            <button className="btn btn-secondary"
              onClick={() => { setEditing(null); setForm({ id: "", name: "", role: "toolmaker", password: "", supervisor: "" }); }}>
              Cancel
            </button>
          )}
        </div>
          ...
          <table>
            ...
            <tbody>
              {employees.map(emp => {
                ...
                const entryCount = entries.filter(e => e.employeeId === emp.id).length;
                return (
                  <tr key={emp.id}>
                    ...
                    <td>
                      <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 11, fontWeight: 700,
                        letterSpacing: 1, textTransform: "uppercase", color: roleColor[emp.role] || "#1a1f2e" }}>
                        {emp.role}
                      </span>
                    </td>
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button className="btn btn-sm btn-secondary"
                          onClick={() => { setForm({ ...emp }); setEditing(emp.id); }}>Edit</button>
                        <button className="btn btn-sm btn-danger"
                          onClick={() => setEmployees(prev => prev.filter(e => e.id !== emp.id))}>Remove</button>
                      </div>
                    </td>
                  </tr>
                );
...
export default function App() {
  ...
  const tabMap = {
    ...
    finance:    [{ id: "finance", label: "Dashboard"    }],
    admin:      [{ id: "admin",   label: "Admin"        }, { id: "finance", label: "Finance View" }],
  };
  ...
  return (
    <div className="app">
      ...
      {tab === "log"     && <ToolmakerForm  user={user} entries={entries} onSubmit={handleSubmit} />}
      {tab === "review"  && <SupervisorView user={user} entries={entries} employees={employees} onUpdate={handleUpdate} />}
      {tab === "finance" && <FinanceDashboard entries={entries} employees={employees} />}
      {tab === "admin"   && <AdminView employees={employees} setEmployees={setEmployees} entries={entries} />}
