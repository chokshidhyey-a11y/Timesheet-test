...
function Footer() {
  return <div style={{ textAlign: "right", padding: "12px 16px", fontSize: 11, color: "#c4c8d4", letterSpacing: 1, borderTop: "1px solid #f0f2f5", marginTop: 16 }}>Developed by: Eurospec</div>;
}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 12 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 10, width: "100%", maxWidth: 680, maxHeight: "90vh", overflow: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.18)" }}>
        ...
        <div style={{ padding: "20px" }}>
          {[
            { role: "👷 Toolmaker / CNC", color: "#fff8e6", border: "#f0dfa0", steps: ["Sign in with your Employee ID and password.", "On first login you'll be asked to set a new password.", "Search for your Project Code, enter hours, check R&D if needed.", "Tap Submit. Toolmaker entries go to your supervisor; CNC entries auto-approve."] },
            { role: "👔 Supervisor", color: "#f0faf0", border: "#c0e0c0", steps: ["Sign in and go to Review tab.", "Tap ✓ to approve, ✕ to reject, or Edit to fix and approve.", "Use Refresh to load new entries.", "Forgot password? Use the Forgot Password link on login."] },
            ...
            { role: "🔧 Admin", color: "#fdf0f0", border: "#f0c0c0", steps: ["Add employees with ID, name, role, and a temporary password.", "All employees must change password on first login.", "Add work email for supervisors/finance/admin for password resets.", "Toggle Show Passwords to see current passwords."] },
          ].map(s => (
    ...
    <div className="login-wrap">
      <div className="login-box" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ background: "#fff8e6", border: "1px solid #f0dfa0", borderRadius: 6, padding: "12px 14px", marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 13, color: "#7a8099", lineHeight: 1.5 }}>Welcome, <strong>{user.name}</strong>! Please set a personal password to continue.</div>
        </div>
        ...
        {sent ? (
          <div>
            <div style={{ background: "#eaf5ea", border: "1px solid #c0e0c0", borderRadius: 8, padding: 16, marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, color: "#4a5068", lineHeight: 1.5 }}>A password reset link has been sent to <strong>{email}</strong>.</div>
            </div>
          ...
          <div>
            <div style={{ background: "#fff8e6", border: "1px solid #f0dfa0", borderRadius: 8, padding: 16, marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, color: "#4a5068", lineHeight: 1.5 }}>Password resets for Toolmakers and CNC operators are handled by your Admin.</div>
            </div>
            <button className="btn btn-primary" style={{ width: "100%" }} onClick={onBack}>Back to Sign In</button>
          </div>
        ) : mode === "email" ? (
          <div>
            <div style={{ fontSize: 13, color: "#7a8099", marginBottom: 16 }}>We'll send a reset link to your work email:</div>
...
function ProjectInput({ value, onChange, projectCodes }) {
  ...
  const matches = projectCodes.filter(p => !query || p.code.includes(query) || (p.description || "").toLowerCase().includes(query.toLowerCase()));
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <input className="form-input" placeholder="Type or search project code..." value={query}
        onChange={e => { setQuery(e.target.value); onChange(e.target.value); setOpen(true); }}
        onFocus={() => setOpen(true)} autoComplete="off" style={{ fontSize: 16 }} />
    ...
    <div className="login-wrap">
      ...
      <div className="login-box" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16 }}>
          <button onClick={() => setShowHelp(true)} style={{ background: "transparent", border: "none", color: "#9aa0b4", fontSize: 12, cursor: "pointer" }}>Need help?</button>
          <button onClick={onForgot} style={{ background: "transparent", border: "none", color: "#c8a84b", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>Forgot Password</button>
        </div>
        <div style={{ marginTop: 20, textAlign: "center", fontSize: 11, color: "#c4c8d4", letterSpacing: 1 }}>Developed by: Eurospec</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="page">
      ...
      <div className="card">
        ...
        {isHigherRole && <div style={{ marginBottom: 12, padding: "8px 12px", background: "#f0f4ff", border: "1px solid #b0c4f0", borderRadius: 4, fontSize: 12, color: "#2a4a9a" }}>This role can reset their password via email.</div>}
        <div style={{ display: "flex", gap: 10 }}>
...
Answer concisely and naturally. Format numbers clearly. If data is not available, say so.`;
  ...
  return (
    <div className="page">
        ...
        <div style={{ height: 420, overflowY: "auto", padding: "20px 20px 12px", display: "flex", flexDirection: "column", gap: 14 }}>
          {messages.map((m, i) => (
            <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", flexDirection: m.role === "user" ? "row-reverse" : "row" }}>
              ...
              <div style={{ maxWidth:"75%", padding:"10px 14px", borderRadius: m.role==="user" ? "12px 4px 12px 12px" : "4px 12px 12px 12px", background: m.role==="user" ? "#1a1f2e" : "#f8f9fc", color: m.role==="user" ? "#fff" : "#1a1f2e", fontSize:13, lineHeight:1.6, border: m.role==="user" ? "none" : "1px solid #e4e7f0", whiteSpace:"pre-wrap" }}>
                {m.content}
              </div>
            </div>
          ))}
          ...
          <div style={{ padding:"0 20px 14px", display:"flex", gap:8, flexWrap:"wrap" }}>
            {suggestions.map(s => (
              <button key={s} onClick={() => { setInput(s); }}
                style={{ background:"#f0f2f5", border:"1px solid #e4e7f0", color:"#4a5068", padding:"6px 12px", borderRadius:20, fontSize:12, cursor:"pointer" }}>
                {s}
              </button>
            ))}
          </div>
      ...
      <div style={{ textAlign:"center", fontSize:11, color:"#c4c8d4", marginTop:8 }}>
